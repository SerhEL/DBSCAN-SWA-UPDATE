...
def get_bac_protein(inputfile_bac,protein_dir,strain_type,save_prefix,add_genome_id='no',kingdom='Bacteria',proteins=''):
	if strain_type == 'fasta':
		...
		annotate_bacteria_fasta(inputfile_bac,protein_dir,save_prefix,kingdom,proteins) #prokka
...
def visualize(save_prophage_file,save_prophage_nucl_file,save_prophage_protein_file,prophage_region_annotate_dir,add_annotation,templeate_file,save_html_file,prefix):
	...
	f_save.write('''<plasmid sequencelength=%s plasmidheight=600 plasmidwidth=800 style='border:0px;'>'''%genome_size)
	f_save.write('''<plasmidtrack trackstyle="fill:#ccc" width="5" radius="160"></plasmidtrack><plasmidtrack trackstyle="fill:rgba(225,225,225,0.5)" radius=150>''')
	counter = 0
	for line in contents[1:]:
...
def predict_prophage(strain_id,inputfile_bac,outdir,add_annotation,prefix,diamond_thread_num,PROTEINS):
	start = time.time()
	...
	get_bac_protein(inputfile_bac,protein_dir,type,prefix,kingdom='Bac_vir',proteins=PROTEINS)
	...
	m2 = threading.Thread(target=predict_prophage_swa,args=(bac_protein_file,bac_protein_blastp_phage_db_file,phage_gene_annotation_dir,20,prefix))
	m1.start()
	m2.start()
	m1.join()
	m2.join()
...
class MyThread(threading.Thread):
	...
	def run(self):
		predict_prophage(self.strain_id,self.inputfile_bac,self.c_outdir,self.add_annotation,self.c_prefix,self.diamond_thread_num,self.ATT_PROTEINS)
...
if __name__=='__main__':
	...
	parser.add_argument('--protein_number', help='optional,the number of expanding proteins when finding prophage att sites, default:10\n')
	parser.add_argument('--min_protein_num', help='optional,the minimal number of proteins forming a phage cluster in DBSCAN, default:6\n')
	...
	parser.add_argument('--proteins',help='''Укажите файл с белками, формат GBK''')
	...
	if len(list(strain_inf_dict.keys()))==1:
		...
		predict_prophage(strain_id,inputfile_bac,outdir,add_annotation,prefix,diamond_thread_num, PROTEINS=ATT_PROTEINS)
	else:
		counter = 0
		tsk = []
		multi_dir = os.path.join(outdir,'results')
